...
export function TechParticles({ scrollProgress }: TechParticlesProps) {
  ...
  const initialPositions = useMemo(() => {
    return sortedSkills.map(() => ({
      x: (Math.random() - 0.5) * INITIAL_SPAWN.RANDOM_SPREAD,
      y: INITIAL_SPAWN.Y_POSITION + (Math.random() - 0.5) * INITIAL_SPAWN.RANDOM_SPREAD,
      z: (Math.random() - 0.5) * INITIAL_SPAWN.RANDOM_SPREAD,
    }));
  }, [sortedSkills]);
  ...
  const finalPositions = useMemo(() => {
    return sortedSkills.map((_, i) => {
      ...
      const offsetX = (Math.random() - 0.5) * CONSTELLATION_LAYOUT.RANDOM_OFFSET;
      const offsetY = (Math.random() - 0.5) * CONSTELLATION_LAYOUT.RANDOM_OFFSET;
  ...
  useFrame((state) => {
    ...
    const rotationSpeed = THREE.MathUtils.lerp(1.0, 0.0, rotationStopFactor);
    groupRef.current.rotation.y =
      (scrollRotation * PARTICLE_ANIMATION.ROTATION.SCROLL_MULTIPLIER +
        state.clock.elapsedTime * PARTICLE_ANIMATION.ROTATION.IDLE_SPEED) *
      rotationSpeed;
  });
